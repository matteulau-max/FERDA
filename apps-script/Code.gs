...
function getTournament() {
  ...
  sessions.forEach(function(s) {
    var entry = { name: s.name, format: s.format, sortOrder: s.sortOrder, courseName: s.courseName, matches: [] };
    if (s.scoring) entry.scoring = s.scoring;
    sessionMap[s.name] = entry;
  });
